...
export const DECISIONS: DecisionEntry[] = [
  ...
  },
  {
    ...
    decision: {
      es: 'Registrar cada orden como una secuencia inmutable de eventos (creada, pagada, retirada) en una base de datos especializada en eventos (EventStore, versión open source). El estado actual se reconstruye repasando la historia, como un libro contable que nunca se borra. Y como con los pagos, hay ruta de escape: empezar con la versión open source y migrar al servicio administrado solo si el volumen lo justifica.',
      en: 'Record every order as an immutable sequence of events (created, paid, picked up) in an event-oriented database (the open-source EventStore). Current state is rebuilt by replaying history, like a ledger written in ink. And as with payments, there is an escape route: start on the open-source version and move to the managed service only if volume justifies it.',
    },
    tradeoff: {
  ...
  },
  {
    ...
    decision: {
      es: 'Generar por adelantado un código PIN de un solo uso para cada entrega. La heladera puede validar el PIN con su memoria local, sin hablar con la nube, y reportar el retiro cuando recupere conexión. Los códigos se guardan también en el propio dispositivo: si una heladera no pudiera validar un código válido, el usuario no queda atrapado en un loop de reintentos.',
      en: 'Pre-generate a single-use PIN for every delivery. The fridge can validate the PIN against its local memory without talking to the cloud, and report the pickup once connectivity returns. The codes are also stored on the user\u2019s own device: if a fridge failed to validate a valid code, the user never gets trapped in a retry loop.',
    },
    tradeoff: {
  ...
  },
  {
    ...
    decision: {
      es: 'La app navega un catálogo guardado localmente (instantáneo y disponible sin señal), y la verificación del stock real se hace en el último paso posible: el momento de pagar. Se acepta mostrar datos posiblemente viejos durante la navegación, nunca durante el cobro. Para que el catálogo local no envejezca, el backend difunde eventos a todos los dispositivos cada vez que algo cambia — y con otra finura: el evento avisa que algo cambió (con id y ubicación del catálogo), no arrastra el catálogo completo; cada dispositivo descarga lo suyo cuando le corresponde. El historial de órdenes, además, vive ~1 mes en el dispositivo para que reordenar lo de siempre sea un toque.<figure class="modal-figure"><img src="/img/IM_meal_stock_update.PNG" alt="Diagrama de información: eventos CatalogUpdated y MealStockUpdated difundidos a todos los usuarios" loading="lazy" /><figcaption>La sincronización: eventos CatalogUpdated y MealStockUpdated difundidos "para todos los usuarios" actualizan el catálogo local de cada dispositivo, que arranca con una descarga completa (Get Catalog). (Documento original de ArchColider)</figcaption></figure>',
