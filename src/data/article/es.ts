  ...
  sections: [
    ...
    /* ───────────────────────── 3 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'callout', tone: 'emerald', title: 'El orden importa', html: 'Fijate que ninguno es filosofía abstracta: cada uno responde a una presión concreta del caso. El equipo chico y el presupuesto mínimo explican la obsesión por la simplicidad; la meta de saltar de 2 a 68 locaciones, la evolucionabilidad; el costo de escalar a ciegas, la telemetría obligatoria; y los sistemas externos que ya existían y no controlaban, los mensajes antes que las llamadas. Los buenos principios no se inventan: <strong>se destilan de las restricciones</strong>. Y el orden importa: cada decisión técnica del resto del artículo se puede rastrear hacia uno de estos cuatro — fijar criterios de desempate <em>antes</em> de discutir tecnologías es lo que evita que la discusión se convierta en guerra de gustos.' },
    ...
    /* ───────────────────────── 4 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/whiteboard-menu-plugins.png', alt: 'Pizarra del equipo: el menú como núcleo rodeado de plug-ins y un post-it sobre el protocolo de mensajería', caption: 'El núcleo "Menu" rodeado de extensiones (nutrición, recomendaciones, reviews, descuentos) — y el post-it que ya anticipaba todo: "se comunica con los plug-ins por protocolo de mensajería (el protocolo debe ser lo bastante inteligente como para desacoplar después); si el núcleo se congestiona, le agregamos cache". Hasta los tamaños crudos están: 4 núcleos/8 GB para el centro, 1 núcleo/2 GB para lo escalado. (Pizarra original de ArchColider)', guide: { title: 'La pizarra del núcleo Menu', html: '<p><strong>Qué es:</strong> una pizarra real de trabajo, sin notación formal — el bosquejo crudo de una sesión de diseño; se lee como una conversación congelada.</p><p><strong>Cómo leerla:</strong> el círculo central "Menu" es el núcleo del sistema; los círculos a su alrededor son extensiones candidatas (N = nutrición, el rayo = recomendaciones, el ojo = reviews, $ = descuentos); los rectángulos celestes son infraestructura: la nube de AWS (4 núcleos / 8 GB) y, aparte, una pieza escalable con balanceador (LB) desplegada como paquete (JAR) de 1 núcleo / 2 GB; MSG = mensajería; las flechas indican quién habla con quién.</p><p><strong>Qué mirar:</strong> el post-it amarillo — mensajería como contrato para desacoplar después, y caché si el núcleo se congestioné. Ahí está la semilla de todas las decisiones que siguen.</p>' } },
        { type: 'p', html: 'Miren el post-it: la filosofía completa del monolito modular, escrita a mano antes de existir ningún diagrama formal. Diseñar para la extracción futura no salió de un libro: salió de una sesión con marcadores — y la composición completa de módulos que esa idea produjo te la muestro dibujada al final de la próxima sección.' },
    ...
    /* ───────────────────────── 5 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/FF_StrategicDomainDesign.jpg', alt: 'Mapa de subdominios estratégicos de DDD', caption: 'El mapa estratégico original del equipo: cada dominio clasificado por unicidad de negocio y complejidad. (Documento original de ArchColider)', guide: { title: 'El mapa estratégico de dominios', html: '<p><strong>Qué es:</strong> un diagrama de clasificación de DDD: cada subsistema del negocio ubicado según dos ejes.</p><p><strong>Cómo leerlo:</strong> el eje vertical mide <em>unicidad</em> (qué tan propio de Farmacy Food es, contra un software genérico); el horizontal, la <em>complejidad</em> de construirlo. Las franjas de color codifican la decisión: verde = Core (Loyalty, Meal Catalog, Ordering) — se construye a medida; amarillo = Supportive (Feedback, Scheduling) — se adapta; celeste = Generic (Reporting, Notifications, Payments) — se alquila. Cada caja es un subsistema completo.</p><p><strong>Qué mirar:</strong> la posición en el mapa ES la decisión de construir-vs-comprar. Un dominio celeste casi nunca merece código propio — por eso pagos y reportes terminan siendo Stripe, Tableau y compañía.</p>' } },
        ...
        { type: 'figure', src: '/img/menu-catalog-acl.png', alt: 'Diagrama del servicio Menu Catalog con su capa anticorrupción', caption: 'El servicio Menu Catalog con su capa anticorrupción (Meals Offer, Loyalty, API): los sistemas externos tocan la frontera, jamás el dominio. Es el diagrama que el propio repo llama "modularización". (Documento original de ArchColider)', guide: { title: 'El servicio Menu Catalog y su aduana', html: '<p><strong>Qué es:</strong> un diagrama de componentes de un solo módulo del monolito, dibujado como si ya fuera un servicio independiente.</p><p><strong>Cómo leerlo:</strong> la caja punteada grande es la frontera del servicio; la franja "Anti-Corruption Layer" (Meals Offer, Loyalty, Menu Catalog API) es la capa de traducción; la caja verde es el dominio puro. A la izquierda, los proveedores con borde punteado (Ghost Kitchen, Loyalty Management, Front End, Point of Sale) — todo lo que entra de afuera. A la derecha, los consumidores internos: Shopping Cart (que sigue hacia el Ordering Service), Recommendations, Reviews y Filtering/Categorization. Las flechas punteadas rotuladas "Commands & Events" significan que no hay llamadas directas: solo mensajes.</p><p><strong>Qué mirar:</strong> ninguna flecha del exterior toca la caja verde: todo pasa primero por la aduana, y el dominio solo habla en comandos y eventos.</p>' } },
        ...
        { type: 'figure', src: '/img/whiteboard-payment-facade.png', alt: 'Pizarra del equipo: fachada de pagos hacia Visa, Mastercard y PayPal', caption: 'La fachada de pagos, garabateada: un solo bloque interno reparte hacia cada red de pago. (Pizarra original de ArchColider)', guide: { title: 'La fachada de pagos garabateada', html: '<p><strong>Qué es:</strong> el sketch de pizarra de cómo conectar las pasarelas de pago.</p><p><strong>Cómo leerla:</strong> la caja de la izquierda es el bloque de pago propio (rotulado "Payment", con MSG recordando que se comunica por mensajes); las cajas que despacha son las redes con las que podría hablar: Visa, MC (Mastercard) y PP (PayPal); las flechas son rutas de cobro.</p><p><strong>Qué mirar:</strong> las redes de pago están DETRÁS de una sola pieza: ningún módulo del sistema habla directo con Visa — todos pasan por la fachada. Cambiar de proveedor = tocar una caja, no el sistema entero.</p>' } },
        ...
        { type: 'figure', src: '/img/FF_Metamodel_v1.png', alt: 'Metamodelo conceptual: nivel de conocimiento y nivel operacional', caption: 'El metamodelo: reglas de negocio (arriba) separadas de las entidades operativas (abajo). Es la respuesta directa a la Entity Trap. (Documento original de ArchColider)', guide: { title: 'El metamodelo: reglamento arriba, partido abajo', html: '<p><strong>Qué es:</strong> un modelo conceptual — no técnico: aquí no hay servidores ni bases de datos, solo las categorías del negocio y sus reglas.</p><p><strong>Cómo leerlo:</strong> la línea punteada horizontal divide dos mundos. Arriba, el <em>Knowledge Level</em>: las REGLAS — User Type (Subscriber, Known, Occasional, PointOfSale), Action Type (Select, Pay, Grab, Schedule, List, Cancel), Order State (el ciclo de vida completo: Started → Finalized → PaymentAwaited → Purchased → Dispatched → Picked, más Scheduled y Canceled), Order Type (Instant, Reservation, Planned), Promotion Type, Meal Type, Feedback Type. Abajo, el <em>Operational Level</em>: los HECHOS — User, Action, Order, Menu, Meal, Schedule, Feedback, Promotion/Discount. Cada flecha con verbo (Can do, Described by, Performed by, Formed from…) dice qué regla de arriba gobierna qué cosa de abajo.</p><p><strong>Qué mirar:</strong> la franja de arriba es el reglamento; la de abajo, el partido. Y no te pierdas Order State: es la columna vertebral del sistema — cada evento de las secciones siguientes mueve la orden de un estado al siguiente.</p>' } },
        ...
        { type: 'p', html: 'El diseño se cerró con una idea que vale copiar: los atributos de calidad globales primero, y después <strong>un presupuesto propio por subsistema</strong> — porque cada pieza necesita patrones distintos. La asignación tiene una lógica reproducible: <strong>preguntar qué le pasa al negocio si esta pieza falla</strong>. Si el catálogo se cae, no hay caja ("sin buena disponibilidad no hay flujo de caja", escriben); si un pago se corrompe, hay pérdida directa. El atributo dominante de cada subsistema es siempre el que protege su parte del negocio. Así quedó el presupuesto: la app, usable, rápida y <em>autónoma</em> (funcionar sin señal); el catálogo, extensible, mantenible y disponible — con una honestidad refrescante: "disponibilidad resuelta con reinicios y escala vertical; una caída corta sigue siendo posible", porque era prueba de negocio, no un banco. Las órdenes: confiabilidad e integridad (es la caja fuerte del dinero). La pasarela de pago: seguridad máxima y disponibilidad ("no poder pagar es pérdida directa de plata"). El diagrama muestra además los dos <em>centros de gravedad</em> destacados, y una elegancia adicional: la app del cajero no es un producto aparte — impersona usuarios y recorre el mismo flujo de trabajo que la app del cliente.' },
        { type: 'figure', src: '/img/FF_system_approach.png', alt: 'Composición del sistema con centros de gravedad y atributos de calidad por subsistema', caption: 'La composición final con los centros de gravedad destacados (Menu Catalog y Ordering) y el presupuesto de calidad de cada pieza. (Documento original de ArchColider)', guide: { title: 'La composición completa y los centros de gravedad', html: '<p><strong>Qué es:</strong> el diagrama de componentes de todo el sistema: qué piezas existen, cómo se agrupan y qué atributo de calidad protege cada grupo.</p><p><strong>Cómo leerlo:</strong> los rectángulos de borde punteado son grupos de despliegue: Front-end apps (Mobile App + Point of Sale), Meal Catalog subsystem (Feedback, Promotion/Discount, Menu Catalog, Meal Pickup), Order processing subsystem (Scheduling + Ordering) y Purchase gateway. Las cajas sólidas son módulos; los dos verdes son los <em>centros de gravedad</em> — el corazón del negocio. Cada grupo anota sus Quality attributes (usability/performance/autonomous; extensibility/maintainability/availability; reliability/integrity; security/availability). Abajo, las cajas punteadas sin color son sistemas externos: se usan, no se controlan.</p><p><strong>Qué mirar:</strong> si entendés los dos verdes (Menu Catalog y Ordering), entendés el 80% del sistema. Y las comunicaciones entre grupos son mensajes, no llamadas — la pizarra de la sección anterior, hecha diagrama.</p>' } },
      ],
    },
    ...
    /* ───────────────────────── 6 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/FF_concurency_order_processing.PNG', alt: 'Diagrama de procesamiento de órdenes con actores', caption: 'Órdenes pasando de actor en actor, cada uno con una sola responsabilidad. (Documento original de ArchColider)', guide: { title: 'Un actor por heladera', html: '<p><strong>Qué es:</strong> un diagrama de concurrencia: quién procesa las órdenes, en qué orden, y por qué nunca chocan.</p><p><strong>Cómo leerlo:</strong> a la izquierda, los clientes agrupados por ubicación (Location A, B, C…). Al confirmar la compra ("User Confirm Order"), la orden entra al "Location" router (celeste = pieza de enrutamiento), que la entrega al MQ (message queue) DE su ubicación. Cada MQ alimenta un solo Fridge (verde = componente), que procesa las órdenes de a una, en orden. Del Fridge salen dos flujos: la Order que sigue su camino, y el "Meal amount updated" hacia el MQ del Catalog — que emite "Catalog updated" hacia todos los clientes.</p><p><strong>Qué mirar:</strong> no existe un stock compartido en una base común: cada heladera tiene su propia fila y su propio "empleado". Sin dos escrituras simultáneas sobre el mismo número, no hacen falta locks.</p>' } },
        ...
        { type: 'figure', src: '/img/IM_meal_purchase.PNG', alt: 'Diagrama de información: compra de una comida paso a paso', caption: 'La compra instantánea de punta a punta: Start Order y Confirm Order (comandos, azul), MealStockReserved y OrderPurchased (eventos, verde). Fíjate cómo el stock reservado y el stock descontado son eventos distintos que escuchan catálogo y reporting. (Documento original de ArchColider)', guide: { title: 'La compra paso a paso: azul promete, verde informa', html: '<p><strong>Qué es:</strong> un diagrama de información: el vocabulario exacto de mensajes de una compra, en orden.</p><p><strong>Cómo leerlo:</strong> la leyenda de la derecha define el alfabeto: azul = Command (una orden de hacer algo), verde = Event (algo que YA ocurrió), amarillo-verde = componente/agregado. La línea vertical punteada separa cliente de servidor; los numeritos (1), (2), (3)… marcan el orden temporal; cada flecha va del emisor al receptor.</p><p><strong>Qué mirar:</strong> Start Order ocurre en el teléfono y no molesta al backend — el servidor solo entra con el Confirm. Del lado servidor, la cadena es MealStockReserved → Catalog → MealStockUpdated difundido "for all users". La regla de oro: azul es promesa, verde es hecho — y todo lo verde queda registrado para siempre.</p>' } },
        ...
        { type: 'figure', src: '/img/IM_cancel_order_by_user.PNG', alt: 'Diagrama de secuencia: cancelación de orden por el usuario dentro de la ventana de inhibición', caption: 'La cancelación dentro de la ventana de inhibición: se resuelve en memoria, sin pasar por la pasarela de pagos. (Documento original de ArchColider)', guide: { title: 'La ventana de deshacer', html: '<p><strong>Qué es:</strong> el mismo alfabeto (azul = comando, verde = evento) aplicado al arrepentimiento impulsivo.</p><p><strong>Cómo leerlo:</strong> igual que la compra: seguí los numeritos. La diferencia está en qué pasa cuando el "Cancel Order by User" llega dentro de la ventana de 10-30 segundos: el System Order resuelve un Meal Stock Canceled y lo propaga al catálogo… sin involucrar a nadie más.</p><p><strong>Qué mirar:</strong> la flecha que NO existe: ninguna línea llega a la pasarela de pago. Esa ausencia ES la táctica — la cancelación muere en memoria, sin comisiones. La versión con pago ya hecho está en el diagrama de cancelación programada.</p>' } },
        ...
        { type: 'figure', src: '/img/user-journey-error.png', alt: 'Journey del usuario cuando la comida queda trabada en la heladera', caption: 'El journey del error: la comida se traba, el usuario documenta con foto, un admin compensa. Diseñar el día nublado también es arquitectura. (Documento original de ArchColider)', guide: { title: 'El journey del día nublado', html: '<p><strong>Qué es:</strong> un diagrama de journey del usuario (pistas por columna): qué experimenta paso a paso cuando la comida se traba.</p><p><strong>Cómo leerlo:</strong> cada columna es un actor o sistema (el usuario, la app, el sistema de órdenes, la heladera, el administrador…); las flechas horizontales son el orden cronológico de la experiencia; el tramo donde el usuario saca una foto es el reclamo documentado; el cruce hacia la columna del admin es la revisión humana; y el cierre es la compensación (comida nueva o cupón).</p><p><strong>Qué mirar:</strong> nadie arregla la heladera por software: el sistema diseña la salida humana del error — evidencia, revisión, compensación. Dibujar ese camino antes del código es lo que separa una arquitectura de un dibujo bonito.</p>' } },
    ...
    /* ───────────────────────── 7 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/whiteboard-subscriber-idea.png', alt: 'Pizarra "IDEA!!!": el concepto de suscripción naciendo', caption: 'El momento en que la suscripción se inventa en la pizarra: menú prepagado por días (1d, 2d, 3d…) con puntos de lealtad. (Pizarra original de ArchColider)', guide: { title: 'La idea del suscriptor naciendo', html: '<p><strong>Qué es:</strong> el sketch rotulado "IDEA!!!": el momento en que el modelo de suscripción se inventa en la pizarra.</p><p><strong>Cómo leerla:</strong> el palito con "+1" y la etiqueta Account es el usuario con cuenta; el cuadro rojo con casilleros A-E es la heladera con comidas disponibles; las flechas numeradas 2 y 3 son elegir y reservar, y el $ es pagar; la franja celeste de abajo es la novedad: el menú prepagado por días (1d, 2d, 3d…) que además suma "+ loyalty points".</p><p><strong>Qué mirar:</strong> la separación física entre las comidas sueltas (arriba) y el menú prepagado (abajo). Esa doble fila es el nacimiento del suscriptor como producto — y de todo su ciclo de eventos.</p>' } },
        ...
        { type: 'figure', src: '/img/IM_preparing_scheduled_orders.PNG', alt: 'Diagrama de información: preparación de órdenes programadas de suscriptores', caption: 'Del calendario a la heladera: PrepareOrders, OrderDispatched, OrderPlacedInFridge y OrderAvailableForPicking — el ciclo completo de una vianda de suscriptor. (Documento original de ArchColider)', guide: { title: 'Del calendario a la heladera', html: '<p><strong>Qué es:</strong> el diagrama de información del ciclo completo de una orden programada de suscriptor.</p><p><strong>Cómo leerlo:</strong> azul = comandos: Get Scheduled Orders (1) consulta la agenda y PrepareOrders (2) le encarga la producción a la cocina. Verde = eventos en cadena causal: OrderDispatched (3) lo escuchan Catalog, Reporting y System Order; OrderPlacedInFridge (4) lo confirma la heladera cuando la comida entra físicamente; y recién entonces OrderAvailableForPicking (5) llega al User.</p><p><strong>Qué mirar:</strong> la causalidad: ningún paso ocurre por arte de magia — cada evento habilita el siguiente, y el usuario solo se entera al final, con su PIN. Si la cocina no despacha, la cadena simplemente no avanza.</p>' } },
        ...
        { type: 'figure', src: '/img/IM_cancel_scheduled_order_by_user.PNG', alt: 'Diagrama de información: cancelación de orden programada con reembolso', caption: 'Cancelación fuera de la ventana: ClaimRefund viaja a la pasarela y RefundSuccessful vuelve a la app. (Documento original de ArchColider)', guide: { title: 'Cancelación con reembolso', html: '<p><strong>Qué es:</strong> el mismo lenguaje de mensajes aplicado a cancelar algo ya programado — el caso que la ventana de 30 segundos no cubre.</p><p><strong>Cómo leerlo:</strong> Get Scheduled Orders (1) muestra lo que existe; Cancel Order by User (2) es el comando; el System Order responde con dos cosas en paralelo: el evento MealStockCanceled (3), que libera stock hacia catálogo y reporting, y el comando ClaimRefund (3), que viaja a Payment; Payment cierra el círculo con el evento RefundSuccessful (4), que vuelve a la app.</p><p><strong>Qué mirar:</strong> dos cosas: la simetría con la compra (comando entra, eventos salen) y que el reembolso es una CONVERSACIÓN de dos mensajes — ClaimRefund y RefundSuccessful — porque "pedir la plata" y "la plata volvió" son hechos distintos.</p>' } },
    ...
    /* ───────────────────────── 8 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/infra-vpc.png', alt: 'Diagrama de la topología de red VPC en AWS', caption: 'La red VPC: dos zonas de disponibilidad, subredes públicas (balanceadores) y privadas (los servidores, sin IP pública). (Documento original de ArchColider)', guide: { title: 'El recinto privado de AWS, con su notación oficial', html: '<p><strong>Qué es:</strong> un diagrama de despliegue con la notación oficial de AWS — donde las cajas anidadas significan contención física o lógica.</p><p><strong>Cómo leerlo:</strong> de afuera hacia adentro: AWS Cloud → Region (la zona geográfica, us-east, la más cercana a Detroit) → VPC (el recinto privado; "10.0.0.0/16" es su rango de direcciones IP internas) → dos Availability Zones (us-east-1a y 1b: edificios de datacenter físicamente separados) → subredes: las verdes son públicas (con salida a internet) y las celestes, privadas (cada una con su rango, p. ej. 10.0.2.0/24). Los círculos naranjas son las puertas: Internet gateway (igw-1, la única salida a internet), Elastic Application Load Balancer (el recepcionista), el Router interno y VGW (la puerta VPN hacia la oficina, "On-Premise").</p><p><strong>Qué mirar:</strong> los servidores viven SOLO en las subredes celestes (privadas); las verdes solo alojan puertas. Y la simetría a/b: si un edificio de datacenter se cae, el otro sigue atendiendo.</p>' } },
        ...
        { type: 'figure', src: '/img/Authentication.png', alt: 'Diagrama del flujo de autenticación con Cognito y ALB', caption: 'El flujo de autenticación: el balanceador valida tokens contra Cognito en el borde de la red. (Documento original de ArchColider)', guide: { title: 'El control de identidades en la puerta', html: '<p><strong>Qué es:</strong> un diagrama de secuencia del login: qué pasa entre el usuario, el balanceador y el proveedor de identidades.</p><p><strong>Cómo leerlo:</strong> el muñeco celeste es el usuario ("Non-Occasional": conocido o suscriptor); los numeritos marcan el orden: (1) llega un pedido sin sesión → (2) el balanceador lo redirige al proveedor de identidades (Cognito, vía OpenID) → (3) el usuario se autentica — con opción de federación (el ícono de Facebook representa a cualquier proveedor, aclaran los autores) → (4) vuelve con sesión autenticada → (5) el balanceador agrega encabezados de identidad y recién entonces reenvía al Auto Scaling group (las N instancias naranjas).</p><p><strong>Qué mirar:</strong> los servidores de la derecha jamás ven una contraseña: confían en los encabezados que les pasa la puerta. Validar una sola vez, en el borde.</p>' } },
        { type: 'figure', src: '/img/services.png', alt: 'Esquema de servicios y hardware virtual: servidores, colas, streaming y SaaS por subred', caption: 'La topología completa: cada servidor es una plantilla t3.medium; colas, streaming de logs y los SaaS externos con su rol. (Documento original de ArchColider)', guide: { title: 'El mapa de máquinas y servicios', html: '<p><strong>Qué es:</strong> el diagrama de despliegue completo: qué corre en qué máquina y qué se alquila afuera.</p><p><strong>Cómo leerlo:</strong> cada caja con forma de servidor es una plantilla de instancia (todas t3.medium); dentro de cada una, los módulos que corren ahí. Las piezas administradas de AWS aparecen como componentes: MQ (Amazon MQ: las colas RabbitMQ gestionadas), Kafka Managed Streams (el stream de logs), SNS (notificaciones push/SMS/email). Afuera del recinto, los SaaS: DataDog (métricas) y Tableau (reportes). Las flechas son canales: colas o HTTPS.</p><p><strong>Qué mirar:</strong> la frontera del recinto separa lo propio (servidores y colas) de lo alquilado (monitoreo y reportes) — la clasificación core/genérico convertida en topología física. Y todo es "plantilla": cualquier caja puede multiplicarse el día que la telemetría lo pida.</p>' } },
        ...
        { type: 'figure', src: '/img/FF_LogBasedStream.PNG', alt: 'Diagrama de propagación de información vía stream basado en log', caption: 'La propagación por log: los productores escriben una vez; reporting, notificaciones y cualquier consumidor futuro leen a su ritmo. (Documento original de ArchColider)', guide: { title: 'El río de logs', html: '<p><strong>Qué es:</strong> el diagrama de flujo de información vía stream de logs — la idea de Kleppmann dibujada.</p><p><strong>Cómo leerlo:</strong> la banda horizontal es el stream: un río de eventos en orden. Las flechas verticales hacia arriba son "post logs" (escribir al río) y hacia abajo "read logs" (leer del río). Arriba, los consumidores que nadie tuvo que avisar: Log Readers que alimentan notificaciones (Email/SMS/Push vía facade) y el reporting (con datos operativos y archivados separados). Abajo, los productores: los módulos con sus atributos de calidad anotados y el subsistema de órdenes completo — Ordering x2, Event Store, Projections; Payment Tracker y Scheduling leen proyecciones; Scheduling confirma a la cocina por HTTPS+Auth. La leyenda lo dice: flecha negra = flujo principal de información.</p><p><strong>Qué mirar:</strong> los productores no saben quién los escucha: escriben al río y cada consumidor lee a su ritmo. Agregar un consumidor mañana no toca a nadie de arriba.</p>' } },
        ...
        { type: 'figure', src: '/img/menu-catalog-extraction.png', alt: 'Caso concreto de extracción: el Menu Catalog como servicio con balanceador y réplicas propias', caption: 'El caso de extracción trabajado: el Menu Catalog ya convertido en servicio, con balanceador propio y N réplicas de filtrado + caché escalando en paralelo. (Documento original de ArchColider)', guide: { title: 'El módulo ya extraído', html: '<p><strong>Qué es:</strong> el MISMO diagrama del Menu Catalog con su aduana, pero después de la extracción como servicio independiente.</p><p><strong>Cómo leerlo:</strong> todo lo que ya conocés sigue igual: la frontera punteada, la capa anticorrupción, el dominio verde, los consumidores a la derecha. La diferencia está abajo: un Load Balancer y N copias idénticas del par Filtering/Categorization + Cache (los "…" indican más réplicas a demanda).</p><p><strong>Qué mirar:</strong> primero qué NO cambió: la frontera y el vocabulario de comandos/eventos son idénticos — extraer no reescribió nada. Después qué cambió: la parte sin estado (filtrar + cachear) se clona detrás de un balanceador para escalar en paralelo. Eso es "diseñado para la extracción" hecho realidad.</p>' } },
    ...
    /* ───────────────────────── 9 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'figure', src: '/img/database_forecast.png', alt: 'Pronóstico de crecimiento de la base de datos', caption: 'El pronóstico de base de datos: ~3,96 GiB mensuales en el escenario proyectado de 30.000 registros. (Planilla original de ArchColider)', guide: { title: 'La proyección de base de datos', html: '<p><strong>Qué es:</strong> un gráfico de la planilla del equipo: cuánto crece la base de datos por mes según el volumen del negocio.</p><p><strong>Cómo leerlo:</strong> los ejes relacionan peticiones por día con almacenamiento acumulado (GiB); las barras corresponden a los escenarios de la planilla (500, 1.000, 5.000 y 10.000 peticiones diarias). El punto relevante: ~30.000 registros al mes ocupan alrededor de 3,96 GiB.</p><p><strong>Qué mirar:</strong> el orden de magnitud: un año entero del negocio cabe en una fracción de disco mínima. Y el propio equipo aclara la honestidad del cálculo: asumió tráfico uniforme — la realidad podría ser hasta un 60% menor.</p>' } },
        { type: 'figure', src: '/img/traffic_forecst.png', alt: 'Pronóstico de tráfico mensual', caption: 'El pronóstico de tráfico del equipo: ~16,5 GiB mensuales en el escenario proyectado. (Planilla original de ArchColider)', guide: { title: 'La proyección de tráfico', html: '<p><strong>Qué es:</strong> la otra cara de la misma planilla: cuántos datos se transfieren por mes según el volumen.</p><p><strong>Cómo leerlo:</strong> mismos escenarios (500 a 10.000 peticiones por día) contra transferencia mensual en GiB. El punto relevante: 1.000 peticiones por día ≈ 16,5 GiB mensuales.</p><p><strong>Qué mirar:</strong> comparalo con el gráfico de base de datos: el tráfico supera al almacenamiento — y la causa no es el negocio: es el usuario que se queja con una foto de 4 MB. La volumetría encuentra esos gigantes escondidos antes de que lleguen a la factura.</p>' } },
        ...
        { type: 'figure', src: '/img/1y-min-tco.png', alt: 'Distribución del costo total anual por servicio', caption: 'La distribución del presupuesto anual: monitoreo y reporting pesan más que la computación. (Documento original de ArchColider)', guide: { title: 'Las tres facturas anuales', html: '<p><strong>Qué es:</strong> tres gráficos de torta: la composición del presupuesto anual en cada escenario de crecimiento (mínimo, proyectado, rápido x10).</p><p><strong>Cómo leerlo:</strong> cada torta es el 100% del costo de un año; cada porción es un servicio, con la leyenda abajo: EC2 (máquinas), Dynamo (base de datos), S3 (archivos), MQ (colas), KMS (llaves), SNS (notificaciones), VPN, Tableau (reportes), DataDog (monitoreo). La primera torta rotula valores en USD; las otras dos, porcentajes.</p><p><strong>Qué mirar:</strong> en el escenario mínimo, DataDog (3.336 USD), DynamoDB (3.072) y Tableau (1.440) juntos pesan más que todas las máquinas (3.115): lo alquilado supera a lo propio. Y crecer x10 no dispara la factura en x10 — la arquitectura barata absorbe el crecimiento.</p>' } },
