  ...
  sections: [
    ...
    /* ───────────────────────── 7 ───────────────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'p', html: 'Y la telemetría tiene un truco extra que vale conocer: los endpoints de salud del monolito exponen tres niveles de información — si cada módulo está <strong>listo para operar</strong>, métricas internas del <strong>negocio</strong> (cómo se procesan los pedidos) y métricas <strong>técnicas</strong> (tasa de requests, tasa de fallos). Además del monitoreo de máquinas, el sistema se prueba con <strong>escenarios sintéticos</strong>: un "cliente dummy" recorre el camino crítico (elegir comida, pagar, retirar) cada tantos minutos y mide si el resultado es correcto y cuánto tarda. Una máquina puede estar "sana" con la lógica de negocio clavada; el cliente fantasma se entera. Esos números son además la señal que decide cuándo partir el monolito.' },
